#!/bin/bash
mkdir -p /G-data && cd /G-data && wget -q -nc https://maskoid.net/data/data.sed
...
echo '##     ##    ##     ##     ######     ##    ##     #######     ####    ########'  
echo
echo '                                                Google Drive Uploder by Maskoid'
echo 
function usage(){

	echo -e "You can upload files & directory to google drive." 
	echo -e "\nUsage:\n $0 [options..] <filename> <foldername> \n"
	echo -e "Foldername argument is optional. If not provided, the file will be upload to preconfigured google drive. \n"
	echo -e "File name argument is optional if create directory option is used. \n"
	echo -e "Options:\n"
	echo -e "-C | --create-directory/folder <folder-name> - option to create directory/folder with folder id."
	echo -e "-r | --root-dir <googledrive-folderID> - google folder id to which the file/directory to upload."
	echo -e "-v | --folder-id - Display Folder ID"
...
function createDirectory(){
    ...
    QUERY=$(echo $QUERY | sed -f /G-data/data.sed)
...
then
	...
	echo -n "Go to $URL and type $USER_CODE to grant access to this application. Hit enter when you done..."
	read
